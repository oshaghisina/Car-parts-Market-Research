    ...
    try:
        ...
        try:
            ...
            if task_id in running_tasks:
                ...
                running_tasks[task_id]['message'] = 'Completed successfully!' if result else 'Failed!'
            
            if result:
                add_log(task_id, '🎉 Scraping completed successfully!', 'success')
                add_log(task_id, f'📊 Found {pipeline.stats.get("final_offers", 0)} offers', 'found')
                add_log(task_id, f'🏪 From {pipeline.stats.get("unique_sellers", 0)} unique sellers', 'found')
            else:
...
def start_scraping():
    ...
    excel_filename = data.get('excel_filename', f'torob_prices_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx')
    
    # Start background task
    thread = threading.Thread(
        target=run_scraping_task,
        args=(task_id, parts_data, excel_filename)
    )
    thread.daemon = True
    thread.start()
