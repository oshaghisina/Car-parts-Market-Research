...
class TorobScraper:
    ...
    def __init__(self, headless: bool = None, delay_range: tuple = None):
        ...
        self.delay_range = (
            delay_range if delay_range is not None else self.config.get_delay_range()
        )
    ...
    async def _scroll_to_load_more(self) -> bool:
        ...
        try:
            ...
            current_products = await self.page.locator(
                self.selectors["product_cards"]
            ).count()
            ...
            new_products = await self.page.locator(
                self.selectors["product_cards"]
            ).count()
    ...
    async def _extract_product_data(self, product_element) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                parent = product_element.locator("..")
                availability_element = parent.locator(
    ...
    async def search_parts(
        self, keywords: str, max_scroll_attempts: int = None
    ) -> List[Dict[str, Any]]:
        """
                        ...
                        if products:
                            ...
                            for product in products:
                                try:
                                    ...
                                    title_raw = product.get("name1", "") or product.get(
                                        "name2", ""
                                    )
                                    ...
                                    # Detect currency
                                    currency_unit = (
                                        detect_currency_unit(price_text)
                                        if price_text
                                        else "unknown"
                                    )
                            ...
                            print(
                                f"Total products extracted from JSON: {len(all_products)}"
                            )
    ...
    async def get_product_details(self, product_url: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(
                f"Successfully extracted {len(product_data['offers'])} offers from product page"
            )
            return product_data
    ...
    async def scrape_part(
        self, part_id: int, part_name: str, part_code: str, keywords: str
    ) -> List[Dict[str, Any]]:
        """
