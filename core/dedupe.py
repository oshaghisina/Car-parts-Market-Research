...
class OfferDeduplicator:
        ...
        for offer in offers:
            ...
            if signature in offer_signatures:
                ...
                if current_score > existing_score:
                    ...
                    deduplicated = [o for o in deduplicated if self._create_offer_signature(o) != signature]
                ...
                for existing_offer in list(offer_signatures.values()):
                    if self._is_similar_offer(offer, existing_offer):
                        ...
                        if current_score > existing_score:
                            ...
                            deduplicated = [o for o in deduplicated if not self._is_similar_offer(o, existing_offer)]
                            deduplicated.append(offer)
    ...
    def normalize_offers(self, offers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for offer in offers:
            ...
            normalized_offer['seller_name_norm'] = self._normalize_seller(offer.get('seller_name', ''))
            
            # Normalize part title
            normalized_offer['title_norm'] = normalize_part_title(offer.get('title_raw', ''))
    ...
    def get_seller_statistics(self, offers: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        """
        ...
        for seller, stats in seller_stats.items():
            ...
            final_stats[seller] = {
                ...
                'sample_urls': ', '.join(list(stats['sample_urls'])[:3]),  # First 3 URLs
...
def test_deduplicator():
    ...
    print(f"\nSeller Statistics:")
    for seller, stats in seller_stats.items():
        print(f"  {seller}: {stats['offers_count']} offers, avg price: {stats['avg_price_toman']:,}")
