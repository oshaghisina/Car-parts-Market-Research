...
from utils.text import convert_rial_to_toman, detect_currency_unit, normalize_seller_name
...
class TorobTwoStagePipeline:
    ...
    async def run_pipeline(self) -> bool:
        ...
        try:
            ...
            async with TorobScraper(headless=self.headless, delay_range=self.delay_range) as scraper:
                for i, part_data in enumerate(self.input_data):
                    ...
                    self.progress_tracker.update_task(f"Searching for {part_name}", f"search_{i}")
                    ...
                    if not search_offers:
                        print(f"   ⚠️  No search results found for {part_data['part_name']}")
                        continue
                    ...
                    filtered_offers = self.filter.filter_search_results(enhanced_search_offers, part_data['keywords'])
                    print(f"   ✓ {len(filtered_offers)} relevant results after filtering")
                    
                    if not filtered_offers:
                        print(f"   ⚠️  No relevant results after filtering for {part_data['part_name']}")
                        continue
                    
                    # Stage B: Product page drill-down
                    self.progress_tracker.update_task(f"Drilling down for {part_name}", f"drill_{i}")
                    product_offers = await self._stage_b_drill_down(scraper, filtered_offers, part_data)
                    self.stats['product_pages'] += len([o for o in product_offers if o.get('drilled_down', False)])
                    self.progress_tracker.complete_task(f"drill_{i}")
                    
                    all_offers.extend(product_offers)
                    self.stats['parts_processed'] += 1
                    
                    print(f"   ✓ {len(product_offers)} offers found for {part_data['part_name']}")
    ...
    async def _stage_a_search(self, scraper: TorobScraper, part_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Stage A: Search page scraping.
        ...
        try:
            ...
            search_results = await scraper.search_parts(
                part_data['keywords'], 
                max_scroll_attempts=3  # Limit scroll attempts
            )
    ...
    async def _stage_b_drill_down(self, scraper: TorobScraper, filtered_offers: List[Dict[str, Any]], part_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
        Stage B: Product page drill-down.
            ...
            try:
                if not offer.get('product_url'):
                    ...
                    enhanced_offer.update({
                        ...
                        'query': part_data['keywords'],
                        'drilled_down': False,
                        'seller_url': offer.get('seller_url', '')  # Preserve seller URL if available
                    })
                    all_offers.append(enhanced_offer)
                    continue
                
                print(f"     📄 Drilling down {i+1}/{max_drill_down}: {offer['title_raw'][:50]}...")
                
                # Get product page details
                product_details = await scraper.get_product_details(offer['product_url'])
                ...
                if product_details and product_details.get('offers'):
                    ...
                    for product_offer in product_details['offers']:
                        enhanced_offer = {
                            ...
                            'seller_name_norm': normalize_seller_name(product_offer.get('seller_name', '')),
                            'price_raw': product_offer.get('price_raw'),
                            'price_text': product_offer.get('price_text', ''),
                            'currency_unit': product_offer.get('currency_unit', 'unknown'),
    ...
    def _calculate_statistics(self, offers: List[Dict[str, Any]]):
        ...
        relevance_scores = [offer.get('relevance', 0.0) for offer in offers if offer.get('relevance')]
        if relevance_scores:
