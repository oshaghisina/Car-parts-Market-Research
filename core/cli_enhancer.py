...
class CLIEnhancer:
    ...
    def show_configuration_summary(self, config_data: Dict[str, Any]) -> None:
        ...
        print(
            f"⏱️  Delay Range: {scraping.get('delay_range', {}).get('min', 'N/A')}-{scraping.get('delay_range', {}).get('max', 'N/A')}s"
        )
        print(
            f"📜 Max Scroll Attempts: {scraping.get('scroll', {}).get('max_attempts', 'N/A')}"
        )
        ...
        print(
            f"🚀 Parallel Processing: {'Enabled' if parallel.get('enabled') else 'Disabled'}"
        )
        ...
        for i, part in enumerate(parts_data, 1):
            rows.append(
                [
                    ...
                    (
                        part.get("keywords", "N/A")[:50] + "..."
                        if len(part.get("keywords", "")) > 50
                        else part.get("keywords", "N/A")
                    ),
                ]
            )
