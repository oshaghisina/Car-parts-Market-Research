...
class ExcelExporter:
    ...
    def __init__(self, output_file: str = None):
        ...
        self.output_file = output_file or self.config.get('export.excel.filename_template', 'torob_prices.xlsx')
    ...
    def _prepare_sellers_summary(self, offers: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for seller_norm, data in seller_groups.items():
            ...
            validation_urls = [f"Product {i+1}: {url}" for i, url in enumerate(sample_urls)]
            validation_url_text = '\n'.join(validation_urls) if validation_urls else 'No URLs available'
    ...
    def _prepare_parts_summary(self, offers: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for part_id, data in part_groups.items():
            ...
            # Prepare URLs for validation
            sample_urls = list(data['urls'])[:5]  # First 5 URLs for better validation
            validation_urls = [f"Offer {i+1}: {url}" for i, url in enumerate(sample_urls)]
            validation_url_text = '\n'.join(validation_urls) if validation_urls else 'No URLs available'
            ...
            row = {
                ...
                'price_std_dev': int(outlier_threshold / 2) if outlier_threshold > 0 else 0,
                'sample_urls': '\n'.join(sample_urls),
                'validation_urls': validation_url_text
            }
                    ...
                    for col_idx, col_name in enumerate(df.columns):
                        if 'price' in col_name.lower() and col_name != 'price_std_dev':
                            for row_idx in range(1, len(df) + 1):  # Skip header
                                ...
                                if cell_value > 0 and abs(cell_value - mean_amp) > outlier_threshold:
                                    worksheet.write(row_idx, col_idx, cell_value, outlier_format)
                                else:
                                    worksheet.write(row_idx, col_idx, cell_value, normal_format)
            ...
            for sheet_name in writer.sheets:
                ...
                if sheet_name == 'offers_raw' and not offers_df.empty:
                    for i, col in enumerate(offers_df.columns):
                        max_len = max(
                            offers_df[col].astype(str).str.len().max(),
                            len(str(col))
                        )
                    ...
                    for i, col in enumerate(sellers_df.columns):
                        max_len = max(
                            sellers_df[col].astype(str).str.len().max(),
                            len(str(col))
                        )
                    ...
                    for i, col in enumerate(parts_df.columns):
                        max_len = max(
                            parts_df[col].astype(str).str.len().max(),
                            len(str(col))
                        )
                        # Special handling for URL columns
                        if col in ['sample_urls', 'validation_urls']:
                ...
                if sheet_name == 'offers_raw' and not offers_df.empty:
                        ...
                        if col_name in offers_df.columns:
                            ...
                            for row_idx in range(1, len(offers_df) + 1):
                                ...
                                if url_value and str(url_value).startswith('http'):
                                    worksheet.write_url(row_idx, col_idx, url_value, url_format, string=str(url_value))
                    ...
                    # Format price columns
                    for col_name in ['AMP_toman', 'median_price_toman', 'min_price_toman', 'max_price_toman']:
                    ...
                    self._apply_conditional_formatting(workbook, worksheet, parts_df, sheet_name)
                ...
                # Enable filters
                if sheet_name == 'offers_raw' and not offers_df.empty:
                    worksheet.autofilter(0, 0, len(offers_df), len(offers_df.columns) - 1)
                elif sheet_name == 'sellers_summary' and not sellers_df.empty:
                    worksheet.autofilter(0, 0, len(sellers_df), len(sellers_df.columns) - 1)
                elif sheet_name == 'part_summary' and not parts_df.empty:
