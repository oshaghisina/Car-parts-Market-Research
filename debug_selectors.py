...
async def debug_selectors():
    ...
    async with TorobScraper(headless=True) as scraper:  # Set to True for headless
        ...
        if card_count > 0:
            ...
            for selector in ['a[href*="/p/"] span', 'a[href*="/p/"]', '.ProductCard_desktop_title__3LVVm', 'h3', 'h4', '.title']:
                try:
            ...
            for selector in ['.ProductCard_desktop_seller__3LVVm', '.seller-name', '.shop-name', '.store-name']:
                try:
                    element = first_card.locator(selector).first
                    count = await element.count()
                    if count > 0:
