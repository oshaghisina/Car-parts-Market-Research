...
def get_user_input():
    ...
    # Show header
    cli.print_header("Torob Two-Stage Automotive Parts Scraper", 
...
def get_multiple_parts_input():
    ...
    # Show header
    cli.print_header("Torob Multi-Part Scraper", 
                    "Process Multiple Parts with Parallel Processing")
    
    # Show configuration summary
    cli.show_configuration_summary(config.config)
    
    # Get number of parts
    num_parts = int(cli.get_user_input(
    ...
    for i in range(num_parts):
        ...
        if i == 0:
            excel_filename = f"{inputs['part_name'].replace(' ', '_').lower()}_prices.xlsx"
...
async def run_pipeline(parts_data, excel_filename):
    ...
    estimated_time = f"{len(parts_data) * 2} minutes" if len(parts_data) > 1 else "2 minutes"
    ...
    try:
        ...
        cli.print_success(f"Pipeline completed successfully! Results saved to {excel_filename}")
        return True
...
async def main():
    ...
    try:
        while True:
            ...
            if choice in ['1', '2']:
                if not cli.get_yes_no("Do you want to process another part?", default=False):
                    cli.print_info("Thank you for using Torob Scraper!")
                    break
